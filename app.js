...
const OTHER_HAZARD_FIELDS = {
  27: { prefix: '19) Other: ', rect: [112, 213, 255, 225] },
  43: { prefix: '27) Other: ', rect: [375, 414, 515, 426] },
  54: { prefix: '38) Other possible hazards: ', rect: [428, 228, 515, 240] },
  55: { prefix: '39) Other: ', rect: [382, 214, 515, 226] }
};
...
function onBodyClick(e) {
  ...
  if (statusBtn) {
    ...
    if (item.status !== 'ok' && item.status !== 'ar') item.otherHazard = '';
    renderSections();
    return;
  }
...
function onBodyInput(e) {
  const detail = e.target.closest('[data-other-hazard]');
  if (!detail) return;
  const [sectionIndex, itemIndex] = detail.getAttribute('data-other-hazard').split(':').map(Number);
  state.sections[sectionIndex].items[itemIndex].otherHazard = detail.value;
}
    ...
    const blocksHtml = section.blocks.map(block => {
      ...
      return `
        <div class="item-row">
          ...
          ${(OTHER_HAZARD_FIELDS[block.exportIndex] && (item.status === 'ok' || item.status === 'ar')) ? `
            <label class="field detail-field">
              <span>Describe other hazard</span>
              <input type="text" data-other-hazard="${sectionIndex}:${itemIdx}" value="${escapeHtml(item.otherHazard || '')}" placeholder="Enter other hazard" />
            </label>
          ` : ''}
...
async function doSave() {
  ...
  try {
    ...
    const hazardDetails = collectHazardDetails();
    const combinedNotes = [state.notes, ...hazardDetails.map(h => `${h.prefix}${h.text}`)].filter(Boolean).join('\n');
    drawMultiline(page, combinedNotes, EXPORT.fields.notes, helv, 10, 12);
    ...
    drawOtherHazardFields(page, flatItems, helv, helvBold);
...
function collectHazardDetails() {
  const details = [];
  state.sections.forEach((sectionState, sectionIndex) => {
    const items = sectionState.items;
    items.forEach((item, itemIndex) => {
      const exportIndex = SECTIONS[sectionIndex].blocks.filter(b => b.type === 'item')[itemIndex].exportIndex;
      const cfg = OTHER_HAZARD_FIELDS[exportIndex];
      if (!cfg) return;
      if ((item.status === 'ok' || item.status === 'ar') && item.otherHazard && item.otherHazard.trim()) {
        details.push({ exportIndex, prefix: cfg.prefix, text: item.otherHazard.trim() });
      }
    });
  });
  return details;
...
function drawOtherHazardFields(page, flatItems, font, fontBold) {
  flatItems.forEach((item, idx) => {
    const cfg = OTHER_HAZARD_FIELDS[idx];
    if (!cfg) return;
    if (!(item.status === 'ok' || item.status === 'ar')) return;
    if (!item.otherHazard || !item.otherHazard.trim()) return;
    drawFieldText(page, item.otherHazard.trim(), cfg.rect, font, 7.5, { align: 'left' });
  });
}
